...
typedef struct {
    ...
    Bit#(32) rv1; 
    Bit#(32) rv2;  
} IssueInst deriving (Eq, FShow, Bits);

typedef struct { 
    Vector#(4, IssueInst) insts;
    Bit#(32) pc;
    Bit#(32) ppc;
    Bit#(1) epoch;    
    Bit#(1) thread_id;
...
module mkPipelined (RVIfc#(n));
    ...
    RWire#(Vector#(4,Bit#(5))) decodeSCPort <- mkRWire;

    Reg#(Bit#(1)) lastFetchedThread <- mkReg(fromInteger(numThreads - 1));
    Reg#(Bit#(2)) fetchWordCtr <- mkReg(0);
...
`endif
	...
	for (Integer thread = 0; thread < numThreads; thread = thread + 1) begin
        ...
        Bit#(32) pc_next = pcs[thread][0] + 16;
...
`endif
    ...
    rule decode if (!starting);
        ...
        let fetchedInstrs = f2d.first();
        let thread = fetchedInstrs.thread_id;
        Vector#(4, Word) line = unpack(x.data);
        Vector#(4, DecodedInst) dinsts;
        Vector#(4, Bit#(5)) rs1_indices <- replicate(0);
        Vector#(4, Bit#(5)) rs2_indices <- replicate(0);
        Vector#(4, Bit#(5)) rd_indices <- replicate(0);
        let all_rdy = True;
        for (Integer i = 0; i < 4; i += 1) begin
            let inst = line[i];
            dinsts[i] = decodeInst(inst);
            rs1_indices[i] = getInstFields(inst).rs1;
            rs2_indices[i] = getInstFields(inst).rs2;
            rd_indices[i] = getInstFields(inst).rd;
            let rs1_rdy <- scs[thread].search(rs1_indices[i]);
            let rs2_rdy <- scs[thread].search(rs2_indices[i]);
            all_rdy = all_rdy && (rs1_rdy || !dinsts[i].valid_rs1) && (rs2_rdy || !dinsts[i].valid_rs2);
        end
...
`endif
    ...
    rule scoreboardInsert if (!starting);
        if (isValid(decodeSCPort.wget())) begin
            Vector#(4, Bit#(5)) zero = replicate(0);
            Vector#(4, Bit#(5)) portVal = fromMaybe(zero, decodeSCPort.wget());
            scs[0].insert(portVal[0]);
            scs[0].insert(portVal[1]);
            scs[0].insert(portVal[2]);
            scs[0].insert(portVal[3]);
        end
    endrule
